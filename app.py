...
def page_intro() -> None:
    st.title("📈 KRX 포트폴리오 & 분석 도구")
    st.markdown(
        "한국 주식 보유 관리 + 추세·모멘텀·밸류·퀄리티 점수판 + 백테스트 + 리스크 관리 + "
        "DART 공시·네이버 뉴스·컨센서스 + 엘리엇 파동까지 **한 화면**에서 보는 무료 도구입니다."
    )

    st.warning(
        "⚠️ **이 사이트의 어떤 점수·신호도 매수/매도 결정의 절대적 근거가 될 수 없습니다.** "
        "보조 참고용으로만 사용하시고, 실제 투자 판단은 본인 책임입니다."
    )

    st.divider()
    st.subheader("✅ 우리가 잘 하는 것")
    st.markdown(
        """
- **한 화면 점검**: 보유 종목을 4가지 관점(추세 / 모멘텀 / 밸류 / 퀄리티)으로 동시에 진단
- **공식 데이터 우선**: KRX(pykrx) · DART(전자공시) · 네이버 금융 — 출처 모두 명시
- **손절가 자동 계산**: ATR 기반 ATR×배수 손절 + 자본 대비 권장 수량 (감정적 매매 줄임)
- **백테스트**: "이 룰을 과거에 썼다면?" 직관적으로 비교 (워크포워드 시기별 진단 포함)
- **포트폴리오 자산배분**: 동일가중 vs 리스크 패리티 비중 비교
- **시장/지수 상대 성과**: KOSPI / KOSDAQ / KOSPI200 / KRX300 비교
- **공시·뉴스·컨센서스**: DART 공시 + 네이버 뉴스 헤드라인 + 애널리스트 목표주가
- **세션 보안**: 포트폴리오·API 키는 본인 브라우저 세션에만 저장
        """
    )

    st.subheader("❌ 무료라서 못 하는 것 (꼭 알아두세요)")
    st.markdown(
        """
- **종목 발굴 X**: 시장 전체 스크리닝 없음 — 보유 종목 + 직접 입력만
- **생존편향**: 상장폐지된 종목이 백테스트에서 빠짐 → 과거 성과가 **항상 과대평가**됨
- **슬리피지**: 거래비용 10bps 고정. 실제 소형주는 1~3% 슬리피지가 흔함
- **통계 유의성 검정 X**: Sharpe 0.8 이 *우연인지 진짜인지* 판별 안 함 (부트스트랩/t-test 없음)
- **거시 환경 미반영**: 금리·환율·업종 사이클 없음
- **재무 깊이**: DART API 키는 받아도 분기 매출·영업현금흐름은 화면에 미연결
- **포트폴리오 상관관계 미시각화**: 리스크패리티만 있고 분산 효과 그래프 없음
- **컨센서스 깊이**: 네이버 스크래핑 — 페이지 구조 바뀌면 깨짐. 애널리스트 수·표준편차 ❌
- **뉴스 알림 X**: "헤드라인 나열"일 뿐. 푸시·이메일·중요도 분류·키워드 필터 ❌
- **실시간성**: 6시간 캐시 — 분 단위 트레이딩 부적합
        """
    )

    st.subheader("📊 솔직한 자체 평가")
    st.markdown(
        "| 영역 | 등급 | 코멘트 |\n"
        "|---|---|---|\n"
        "| 데이터 정확성 | **A-** | KRX·DART 공식 데이터. 네이버 스크래핑은 깨질 위험 |\n"
        "| 추세 / 모멘텀 신호 | **B** | 학계 검증된 시그널 (Moskowitz, Jegadeesh-Titman) |\n"
        "| 밸류 + 퀄리티 점수 | **B-** | 가중치(35/30/20/15)는 임의값, 최적화 X |\n"
        "| 백테스트 | **C+** | 단일 종목 · 슬리피지·생존편향 미반영 |\n"
        "| 포트폴리오 | **B-** | 리스크패리티 + 지수 비교 추가됨 |\n"
        "| 엘리엇 파동 | **C-** | 학술 검증 거의 없음 — 참고용 |\n"
        "| **종합** | **B- ~ B** | *무료 도구 중 상위 15~20%* 수준 |\n"
    )

    st.subheader("✅ 이렇게 쓰세요")
    st.markdown(
        """
1. **종목 점수판**: 보유 종목 중 "추세 깨진 게 있나?" 빠른 점검
2. **백테스트**: 룰이 과거에 통했는지 감 잡기 (성능 보장 X)
3. **리스크 관리**: 손절가 정해놓고 *규율* 유지하는 용도
4. **공시·뉴스**: 보유 종목 이슈 빠르게 훑기
        """
    )

    st.subheader("❌ 이렇게 쓰지 마세요")
    st.markdown(
        """
- "종합 75점이니까 무조건 매수" — 단일 점수만 의존
- "엘리엇 5파 목표가 도달 예측" — 휴리스틱의 한계 인지 필요
- 실시간 단타 매매의 근거 (캐시 6시간)
- 본인 자산의 큰 비중을 거는 결정
        """
    )
    ...
    st.markdown(
        ...
        "|---|---|---|\n"
        "| **이 사이트** | B- ~ B | 무료 |\n"
        "| 네이버 금융 / 인베스팅닷컴 | B (정보량) | 무료 |\n"
        "| 증권사 HTS (영웅문 등) | B+ | 무료 (계좌개설) |\n"
        "| 퀀티와이즈 / 한경컨센서스 | A- | 유료 |\n"
        "| Bloomberg / FactSet | A+ | 연 2천만원+ |\n"
    )

    st.divider()
    st.info(
        "👉 왼쪽 사이드바에서 메뉴를 선택해 시작하세요. "
        "처음이라면 **내 포트폴리오** 에 보유 종목을 추가한 뒤 **종목 점수판**을 보는 흐름을 추천합니다."
    )
    st.caption(
        "데이터: pykrx (KRX 공식) · DART (전자공시) · 네이버 금융  ·  "
        "🔒 포트폴리오·API 키는 본인 브라우저 세션에만 저장됨"
    )
