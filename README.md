...
- **포트폴리오 관리**: 종목 추가·합산(가중평단)·삭제, 실시간 평가손익, JSON import/export
- **종목 점수판**: 추세(35%) · 모멘텀(30%) · 밸류에이션+퀄리티(20%) · 변동성(15%) 가중 종합 신호 (매수/관망/매도)
  - 네이버 금융에서 ROE / 부채비율 / 영업이익률 / 선행 PER 까지 수집
  - **컨센서스 목표가 + 상승여력** 인라인 표시
- **시장 비교**: 종목 vs KOSPI/KOSDAQ/KOSPI200 상대 성과, **리스크 패리티 권장 비중** + 동일가중·현재 비중과 백테스트 비교
- **백테스트**: "200일 이평선 위 + 12개월 모멘텀 > 0" 규칙 + **시기별 분할 워크포워드** 진단
- **리스크 관리**: ATR(14) 기반 손절가 + 거래당 리스크 % 기반 권장 수량 산출
- **공시 & 뉴스**:
  - DART 공식 공시 목록 (API 키 필요, [opendart.fss.or.kr](https://opendart.fss.or.kr) 무료)
  - 네이버 뉴스 헤드라인 + 링크
  - 컨센서스 목표주가 / 투자의견 / 상승여력
- **엘리엇 파동 분석**:
  - ZigZag(여러 임계값 자동 탐색)로 스윙 포인트 추출
  - 마지막 9개 피벗을 5-임펄스 + ABC 조정으로 라벨링
  - 엘리엇 3대 규칙 검증 + 피보나치 비율 적합도
...
공시 페이지를 활성화하려면:
1. <https://opendart.fss.or.kr> 가입 (1분, 본인인증)
2. 마이페이지 → 인증키 신청 → 즉시 발급
3. 앱 사이드바 "⚙️ DART API 키" 에 입력 (본인 세션에만 저장, 서버 영구 저장 X)
...
│   ├── signals.py          추세/모멘텀/변동성/밸류 점수
│   ├── backtest.py         룰 백테스트 + 시기별 분할(워크포워드)
│   ├── risk.py             ATR 손절가 + 포지션 사이징
│   ├── benchmark.py        KOSPI/KOSDAQ 지수 + 상대 성과
│   ├── portfolio_optim.py  리스크 패리티 / 동일가중 / 최소분산
│   ├── dart.py             DART 공식 공시 API 클라이언트
│   ├── naver_finance.py    네이버 펀더멘털(PER/ROE/...) 스크래퍼
│   ├── naver_extras.py     네이버 컨센서스 목표가 + 뉴스 스크래퍼
│   ├── portfolio.py        보유 종목 자료구조/직렬화
│   └── charts.py           Plotly 차트
├── data/
